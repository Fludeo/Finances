...
class AuthService {
    ...
    login(userLogin, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(yield comparePassword(userLogin.password, checkUser.hash))) {
...
function generateAccessToken(user) {
    const accessToken = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, String(process.env.ACCESS_TOKEN_SECRET), { expiresIn: 60 * 15 });
    return accessToken;
}
function generateRefreshToken(user) {
    const accessToken = jsonwebtoken_1.default.sign({ id: user.id, email: user.email }, String(process.env.REFRESH_TOKEN_SECRET), { expiresIn: 60 * 60 * 24 * 7 });
    return accessToken;
}
function comparePassword(hash1, hash2) {
    return __awaiter(this, void 0, void 0, function* () {
        const result = yield bcrypt_1.default.compare(hash1, hash2);
        return result;
    });
}
